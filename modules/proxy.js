...
function collectResponseBody(proxyRes) {
    return new Promise((resolve, reject) => {
        ...
        stream.on('error', err => {
            // Jika dekompresi gagal, fallback ke raw body
            console.log(`[PROXY] Decompress error (${encoding}): ${err.message}, trying raw`);
            const rawChunks = [];
            proxyRes.on('data', c => rawChunks.push(c));
            proxyRes.on('end', () => resolve(Buffer.concat(rawChunks)));
