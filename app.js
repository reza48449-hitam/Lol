...
// File patch di-serve dari root project directory
// Game request ini langsung tanpa prefix /cdn/
app.get('/Assembly-CSharp-patch.bytes', (req, res) => {
    const filePath = path.join(__dirname, 'Assembly-CSharp-patch.bytes');
    if (!fs.existsSync(filePath)) {
        console.log('[PATCH] Assembly-CSharp-patch.bytes NOT FOUND at', filePath);
        return res.status(404).send('Not Found');
    }
    console.log('[PATCH] Serving Assembly-CSharp-patch.bytes');
    res.setHeader('Content-Type', 'application/octet-stream');
    res.setHeader('Cache-Control', 'public, max-age=3600');
    res.sendFile(filePath);
});

// Also serve under codepatch path (fallback)
app.get('/cdn/live/ABHotUpdates/gameassetbundles/codepatch/assembly-csharp-patch.bytes', (req, res) => {
    const filePath = path.join(__dirname, 'Assembly-CSharp-patch.bytes');
